...
| 并发隔离 | 线程并行派工（daemon 工作线程 + 信号量），单工人失败/超时以文本如实带回，不拖垮整体 |
| 同站串行（一站同时只跑一个模型） | `collaboration.max_per_station`（默认 **1**）：同一中转站（`base_url`+`api_key`）同时允许在飞的请求数。门卡在**真正发请求**的那一层（`WorkerPool._chat_gated`，探测走 `_probe_one` 同一口径），因此批量派工、顺序 `ask`、失败回退（含跨站回退）、延迟探测共用一个约束，不存在"批量串行但回退偷偷并发第二发"的漏口。为什么默认串行：一站多模型只是同一份配额/同一条网关上的多个入口，并发打过去只在站端互相排队拖慢（实测同站 3 路并发时单模型耗时数倍膨胀），更会把「延迟档案」——选路裁剪的依据——污染成"这个模型慢"，实际是站被自己人堵了。串行后并行度 = `min(max_workers, 每站配额 × 本批涉及站数)`，**跨站点名才提速**；想要旧行为把本值调大即可。门只包一次 `chat`、**回退递归发生在门外**：持着 A 站的门去等 B 站的门，两个工人反向回退会 A 等 B、B 等 A 死锁。延迟探测遇站被占最多等 2s 即让路（本轮跳过该工人），绝不为个别样本陪等真实派工的 120s。真实并行度随 `health` 指令的 `concurrency` 字段带回（`stations`/`max_workers`/`max_per_station`/`effective`），主据此决定一批点几个、分几轮 |
| 整体限时 | `ask_many`/`vote`/`collect` 用 `concurrent.futures.wait` 对收集阶段整体限时；限时**按规模自适应** = `ceil(参与人数 ÷ 实际并行度) × llm.timeout + 30s`（调用方显式传入 `timeout` 时以传入值为准）。实际并行度按上一条折算：传了工人名单时 = `min(max_workers, max_per_station × 本批涉及站数)`，不传名单退回旧的 `min(max_workers, n)` 口径。原先固定 300s 在「池大 + 并发低」时必然截断尾部批次（25 名 / 3 并发 = 9 批，单工人耗时 >33s 即超标）；同站串行后排队更久，限时若仍按 `max_workers` 算会把"排队被判未完成"的额度浪费重新引入。超时工人标记"未完成"放弃等待，后台线程按 LLM 超时自行收尾 |
